...
<?php 
	if(isset($_GET['delete'])) {
		...
		if($conn->query($delete_from_procat) && $conn->query($delete_products)) {
			adminMessage('green', 'Successfully Deleted Category');
		} else {
							...
							<div class="my-div">
								...
								<div id="product-query">
									...
									<?php
										...
										while($row_main = $result_main->fetch_array()) {
									?>
										<select onchange="updateProductId(this.value)" id="<?php echo htmlspecialchars($row_main['main'])."-products" ;?>" style="display: none" class="form-control">
									...
									<script>
										function updateProductId(id) {
											var confirmation = confirm("Want To Deltete This Category ?");
